...
class Validator:
    ...
    def warn(self, message):
        ...
        if self.ask:
            answers = prompt(
                [{"type": "confirm", "message": "Proceed anyway?", "name": "okay"}]
            )
            if not answers["okay"]:
                exit()
    ...
    def from_header(validator: Validator, component: str, header: str):
        ...
        if component == "NSPR":
            return PackageVersion(
                "NSPR",
                PackageVersion.extract_version(
                    header, regex=RE_nspr_version, validator=validator
                ),
            )
...
class Patch:
    ...
    def verify_tag_version(self, *, validator: Validator, version: PackageVersion):
        ...
        validator.info(
            f"Tag {tag} for version {version.number} detected. Format looks good."
        )
